...
RSA is known to yield inaccurate results in scalar implicatures and absolute adjectives [cite]. In the case of scalar implicature, it is argued that listeners are insensitive to prior manipulations [cite]. Taking the example from [cite], if we present subjects with the scenario: Someone threw 10 blocks of marble in the water; John, who witnessed the event, told you that 'some marbles sank', then RSA supposedly predicts that the interpretation is most likely that all marbles sank. But this is not necessarily the case. Note that in S1, the KL-divergence this interpretation yields tends to 0. Therefore, the probability of this interpretation at L1 would also tend to zero. In other words, if our world-state prior predicts that 100% of chance all marbles would sink, then an interpretation that exactly matches this prior would yield no reduction of entropy whatsoever, as there is no room for reduction in the first place. At this point, the speaker becomes indifferent between "some" and "all", as the utility of both tends to 0. If the pargmatic listener forces an interpretation, then [equations to be inserted]. The question is whether the listener even attempts an interpretation in this case. A variant of RSA, the wonky world model [cite], introduce a lifted variable w that switches the world-state prior distribution between skewed(reflects what the listener expects in a regular world) and flat(reflects a wonky world where all states are equally possible): <br>
